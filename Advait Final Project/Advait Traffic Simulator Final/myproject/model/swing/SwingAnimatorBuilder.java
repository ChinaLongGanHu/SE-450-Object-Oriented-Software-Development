...
public class SwingAnimatorBuilder implements AnimatorBuilder {
	...
	private static double skipRoad = VP.gap + MP.varRoadLength;
	...
	public void addLight(Light d, int i, int j) {
		...
		Translator t = new TranslatorWE(x, y, MP.getvarCarLength(),
	...
	public void addHorizontalRoad(Road l, int i, int j, boolean eastToWest) {
		...
		double y = skipInit + skipRoad + i * skipRoadCar;
		Translator t = eastToWest ? new TranslatorEW(x, y, MP.varRoadLength,
				VP.elementWidth, VP.scaleFactor) : new TranslatorWE(x, y,
				MP.varRoadLength, VP.elementWidth, VP.scaleFactor);
		painter.addRoad(l, t);
	}
	...
	public void addVerticalRoad(Road l, int i, int j, boolean southToNorth) {
		...
		Translator t = southToNorth ? new TranslatorSN(x, y, MP.varRoadLength,
	...
	private static class MyPainter implements SwingAnimatorPainter {
		...
		public void paint(Graphics g) {
			...
			for (Element<Light> e : lightElements) {
				...
				XGraphics.fillRect(g, e.t, 0, 0, MP.varCarLength,
						VP.elementWidth);
			}

			Color ROADCOLOR = new Color(192, 192, 192);
			g.setColor(ROADCOLOR);
			for (Element<Road> e : roadElements) {
				XGraphics.fillRect(g, e.t, 0, 0, MP.varRoadLength,
						VP.elementWidth);
			}
			...
			for (Element<Road> e : roadElements) {
				...
				for (Car d : e.x.getCars().toArray(new Car[0])) {
					...
					XGraphics.fillOval(g, e.t, d.getPosition(), 0,
							MP.varCarLength, VP.elementWidth);
